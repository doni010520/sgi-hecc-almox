...
export function RequestPending() {
  ...
  const getRequestStats = () => {
    ...
    return { total, pending, approved, pharmacy, warehouse, urgent, today }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h1 className="inline-flex items-center gap-2 flex-wrap text-2xl font-bold text-gray-900">Pendências <ActiveStockBadge /></h1>
            <p className="text-sm text-gray-500 mt-1">
              Solicitações aguardando análise ou processamento
            </p>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as typeof activeTab)}>
          ...
          <TabsContent value="pending" className="p-0">
            <div className="divide-y divide-gray-100">
              {filteredRequests.filter(r => r.status === 'pending').length > 0 ? (
                filteredRequests.filter(r => r.status === 'pending').map((request, index) => renderRequestCard(request, index))
              ) : (
                <div className="p-8 text-center">
                  <p className="text-gray-500">Nenhuma solicitação pendente</p>
                </div>
          ...
          <TabsContent value="approved" className="p-0">
            <div className="divide-y divide-gray-100">
              {filteredRequests.filter(r => r.status === 'approved').length > 0 ? (
                filteredRequests.filter(r => r.status === 'approved').map((request, index) => renderRequestCard(request, index))
              ) : (
                <div className="p-8 text-center">
                  <p className="text-gray-500">Nenhuma solicitação aprovada aguardando processamento</p>
                </div>
              )}
            </div>
          </TabsContent>
          ...
          <TabsContent value="pharmacy" className="p-0">
            <div className="divide-y divide-gray-100">
              {filteredRequests.filter(r => r.type === 'pharmacy').length > 0 ? (
                filteredRequests.filter(r => r.type === 'pharmacy').map((request, index) => renderRequestCard(request, index))
              ) : (
                <div className="p-8 text-center">
                  <p className="text-gray-500">Nenhuma solicitação de farmácia pendente</p>
                </div>
              )}
            </div>
          </TabsContent>
          ...
          <TabsContent value="warehouse" className="p-0">
            <div className="divide-y divide-gray-100">
              {filteredRequests.filter(r => r.type === 'warehouse').length > 0 ? (
                filteredRequests.filter(r => r.type === 'warehouse').map((request, index) => renderRequestCard(request, index))
              ) : (
                <div className="p-8 text-center">
                  <p className="text-gray-500">Nenhuma solicitação de almoxarifado pendente</p>
                </div>
              )}
            </div>
          </TabsContent>
